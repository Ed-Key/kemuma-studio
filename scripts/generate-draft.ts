// CLI: generate a listing draft for one design.
// Usage: npx tsx scripts/generate-draft.ts <design_id> [provider:model]
...
import { createClaudeWriter, generateDraft } from '../src/lib/writer/generate'
import { createWriterFor } from '../src/lib/writer/providers'

async function main() {
  const designId = Number(process.argv[2])
  if (!Number.isFinite(designId)) throw new Error('usage: tsx scripts/generate-draft.ts <design_id> [provider:model]')
  const spec = process.argv[3]
  const writer = spec ? createWriterFor(spec) : createClaudeWriter()
  const db = getCatalogDb()
  const detail = getDesignDetail(db, designId)
  if (!detail) throw new Error(`design ${designId} not found`)
  console.log(`generating draft for "${detail.name}" with ${writer.label}...`)
